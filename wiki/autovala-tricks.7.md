...
To access the Vala functions from C, just include the corresponding header files. Each one is called like its corresponding **.vala** file, so if you have **example.vala**, just include **example.h**. During compilation, cmake will add **-I.../install/src/...**, so the **.h** files created by **valac** will be available to the **.c** files.
